...
class TripppleTools{
  ...
  constructor(){
    this.VERSION =jsonfile.readFileSync(VERSION_FILE);
  ...
  bump(opts={}){
    return new Promise((resolve, reject) => {
      ...
      jsonfile.writeFileSync(VERSION_FILE, result, {spaces: 2}, (err) => {
  ...
  deploy(opts={}){
    ...
    if(ENV == 'production'){
      ...
      console.log(error(`local deploy ${this.VERSION} ${timestamp}`));
    }
