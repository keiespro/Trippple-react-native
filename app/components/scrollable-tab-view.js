...
const ScrollableTabView = React.createClass({
    ...
    getInitialState() {
        let w = this.props.startPage ? (this.props.padded ? MagicNumbers.screenWidth : DeviceWidth) / this.props.children.length : 0
    ...
    goToPage(pageNumber) {
      ...
      this.props.onChangeTab({i: pageNumber, ref: this.props.children[pageNumber]});
        let w = this.state.width / this.props.children.length
