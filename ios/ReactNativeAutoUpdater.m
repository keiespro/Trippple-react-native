...
- (NSDictionary *)constantsToExport {
    ...
    if (lastUpdateCheckPerformedOnDate) {
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd-HH:mm:ss"];
        lastCheck = [dateFormatter stringFromDate:lastUpdateCheckPerformedOnDate];
    }
    NSString* version = @"";
    if (metadata) {
...
- (void)performUpdateCheck {
    ...
    NSLog(@"Checking url %@",[self.metadataUrl absoluteString]);
    if (!data) {
        NSLog(@"Received no Update Metadata. Aborted");
        if (self.showProgress) {
    ...
    if (error) {
        NSLog(@"Error reading Metadata JSON. Update aborted");
...
- (void)checkUpdate {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        NSLog(@"[ReactNativeAutoUpdater]: Checking update");
        if (self.metadataUrl) {
...
-(void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    ...
    if ([data writeToFile:filename atomically:YES]) {
        ...
        if ([self.delegate respondsToSelector:@selector(ReactNativeAutoUpdater_updateDownloadedToURL:)]) {
            ...
            NSLog(@"[ReactNativeAutoUpdater] NEW JS file://%@", filename);
