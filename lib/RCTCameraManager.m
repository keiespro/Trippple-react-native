...
- (CGImageRef)newCGImageRotatedByAngle:(CGImageRef)imgRef angle:(CGFloat)angle
{
    ...
    CGContextRef bmContext = CGBitmapContextCreate(NULL,
                                                  ...
                                                  colorSpace,
                                                  (CGBitmapInfo)
                                                  CGAffineTransform transform = CGContextGetCTM(bmContext);
    // left mirror
    transform = CGContextTranslateCTM( bmContext, width, 1);
     transform = CGContextRotateCTM( bmContext, M_PI_2 );
      transform = CGContextTranslateCTM( bmContext, width, 1);
       transform = CGContextScaleCTM( bmContext, -1, 1);


    CGContextConcatCTM(bmContext, transform);
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    CGContextDrawImage(bmContext, CGRectMake(-width,-height,width,height),imgRef);
